  ...
  return (
    <div className="min-h-screen bg-[linear-gradient(180deg,#eef4ff_0%,#f7faff_42%,#f3f6fb_100%)] text-[color:var(--color-ink)]">
      <div className="mx-auto flex max-w-[1800px] gap-4 px-3 py-3 sm:px-4 lg:px-5">
        <aside className="hidden w-[236px] shrink-0 rounded-[28px] bg-[linear-gradient(180deg,#06173f_0%,#0a1e53_52%,#10265f_100%)] px-3 py-4 text-white shadow-[0_26px_58px_rgba(9,20,56,0.24)] lg:flex lg:flex-col">
          ...
          <nav className="mt-8 grid gap-2">
            {navItems.map((item, index) => (
              <div
                className={cx(
                  'flex min-h-13 items-center gap-3 rounded-[14px] px-3 py-2.5 text-[0.96rem] font-medium',
                  index === 0
        ...
        <div className="min-w-0 flex-1 space-y-4">
          ...
          <div className="rounded-[28px] border border-[color:var(--color-border)] bg-white/92 px-5 py-5 shadow-[0_18px_36px_rgba(19,33,79,0.08)]">
            <LoadingState
              title="Validando sessao do suporte"
              description="O workspace principal aparece assim que a sessao Supabase confirmar o acesso operacional."
            />
          </div>
        </div>
      </div>
